...
export default function AdminDashboard() {
  ...
  const handleImagesChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    Promise.all(readers).then((results) => setMedia((m) => ({ ...m, images: [...m.images, ...results] })));
  };
  ...
  const saveBlog = () => {
    if (editingIdx !== null) {
      ...
      const posts = [...savedPosts, { title, content, author, date: date || new Date().toISOString(), category, media }];
      setSavedPosts(posts);
  ...
  if (!authorized)
    return (
      <main className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 p-6">
        <form onSubmit={handleLogin} className="bg-white dark:bg-gray-800 p-10 rounded-2xl shadow-lg w-full max-w-sm">
          <h2 className="text-2xl font-semibold text-gray-800 dark:text-gray-100 mb-6 text-center">Admin Login</h2>
          <input
            type="password"
          ...
          />
          <button className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-semibold transition">Login</button>
        </form>
      </main>
    );
            ...
            return (
              <button
                ...
                className={`relative text-left w-full p-3 rounded-lg font-medium transition flex items-center ${
                  isActive ? 'bg-blue-50 dark:bg-gray-700 text-blue-600 dark:text-blue-400' : 'hover:bg-gray-100 dark:hover:bg-gray-700'
                }`}
              >
                {isActive && <span className="absolute left-0 top-0 h-full w-1 bg-blue-600 dark:bg-blue-400 rounded-tr-lg rounded-br-lg"></span>}
                {label}
              </button>
            );
          })}
              ...
              {filteredPosts.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {filteredPosts.map((post, idx) => {
                    ...
                    return (
                      <motion.div key={idx} className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 relative">
                        <h3 className="text-xl font-semibold mb-1">{post.title}</h3>
                        <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">By {post.author}</p>
